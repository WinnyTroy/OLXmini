...
def index():
    inventory = var.query(Inventory).all()
...
def view_particulars(inventories_id):
    inventory = var.query(Inventory).filter_by(inventories_id=inventories_id).one()
...
# setting up an instance of the tables that we are going to run, data we will need to store.
        users = Users(name=name, mobile_number=number,neighbourhood=neigh, email_address=email)
        inventory = Inventory(p_title=title, p_desc=desc, p_price=25, var=users)
